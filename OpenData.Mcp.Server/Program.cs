...
public static class UKParliamentOpenDataTool
{
    ...
    public static async Task<string> GetMemberByNameAsync(IServiceProvider serviceProvider, string name)
    {
        ...
        return await GetResult(serviceProvider, url);
    }
    ...
    [McpServerTool, Description("whatson in the calendar")]
    public static async Task<string> SearchCalendar(IServiceProvider serviceProvider, string house, string startDate, string endDate)
    {
        var url = $"https://whatson-api.parliament.uk/calendar/events/list.json?queryParameters.house={house}&queryParameters.startDate={startDate}&queryParameters.endDate={endDate}";
        return await GetResult(serviceProvider, url);
    }

    [McpServerTool, Description("get list of sessions")]
    public static async Task<string> GetSessions(IServiceProvider serviceProvider, string house, string startDate, string endDate)
    {
        var url = $"https://whatson-api.parliament.uk/calendar/sessions/list.json";
        return await GetResult(serviceProvider, url);
    }

    [McpServerTool, Description("get list of non sitting periods")]
    public static async Task<string> GetNonSittingDays(IServiceProvider serviceProvider, string house, string startDate, string endDate)
    {
        var url = $"https://whatson-api.parliament.uk/calendar/events/nonsitting.json?queryParameters.house={house}&queryParameters.startDate={startDate}&queryParameters.endDate={endDate}";
        return await GetResult(serviceProvider, url);
    }

    public static async Task<string> GetResult(IServiceProvider serviceProvider, string url)
    {
